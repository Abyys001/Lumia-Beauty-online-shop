...
from apps.accounts.sms import get_sms_provider, get_sms_provider_for_profile
...
from apps.admin_api.serializers import (
    ...
    AdminSmsProviderProfileSerializer,
    AdminSmsProviderSettingsSerializer,
)

VALID_PROVIDER_TYPES = {
    SmsProviderProfile.PROVIDER_MOCK,
    SmsProviderProfile.PROVIDER_SMSIR,
    SmsProviderProfile.PROVIDER_IRANPAYAMAK,
}
...
def _default_base_url(provider_type: str) -> str:
    return SmsProviderProfile.default_base_url(provider_type)


def _profile_response_data(profile: SmsProviderProfile) -> dict:
    data = AdminSmsProviderProfileSerializer(profile).data
    data['api_key'] = SmsConfigService.mask_api_key(profile.api_key_encrypted)
    data['sandbox_api_key'] = SmsConfigService.mask_api_key(profile.sandbox_api_key_encrypted)
    data['panel_password'] = SmsConfigService.mask_panel_password(profile.panel_password_encrypted)
    data['has_api_key'] = bool(
        SmsConfigService.resolve_iranpayamak_api_key(profile=profile)
        if profile.provider_type == SmsProviderProfile.PROVIDER_IRANPAYAMAK
        else SmsConfigService.resolve_api_key(profile=profile, sandbox=False)
        or SmsConfigService.resolve_api_key(profile=profile, sandbox=True)
    )
    data['has_bearer_token'] = bool(SmsConfigService.resolve_bearer_token(profile=profile))
    return data


def _apply_profile_settings(request, profile: SmsProviderProfile, *, validate_keys: bool = True) -> Response | None:
    data = dict(request.data)
    api_key = data.pop('api_key', None)
    sandbox_api_key = data.pop('sandbox_api_key', None)
    panel_password = data.pop('panel_password', None)
    was_sandbox = profile.is_sandbox

    serializer = AdminSmsProviderProfileSerializer(profile, data=data, partial=True)
    serializer.is_valid(raise_exception=True)
    serializer.save()

    if profile.provider_type == SmsProviderProfile.PROVIDER_IRANPAYAMAK:
        profile.base_url = _default_base_url(SmsProviderProfile.PROVIDER_IRANPAYAMAK)
        profile.save(update_fields=['base_url', 'updated_at'])
        sync_iranpayamak_default_template()
    elif profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR:
        if not profile.base_url or 'iranpayamak' in profile.base_url:
            profile.base_url = _default_base_url(SmsProviderProfile.PROVIDER_SMSIR)
            profile.save(update_fields=['base_url', 'updated_at'])

    base_url = profile.base_url or _default_base_url(profile.provider_type)
    ...
    if api_key is not None and not _is_masked_key(str(api_key)):
        ...
        SmsConfigService.set_api_key(cleaned, sandbox=False, profile=profile)

    if sandbox_api_key is not None and not _is_masked_key(str(sandbox_api_key)):
        cleaned = str(sandbox_api_key).strip()
        if validate_keys and profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR:
            _, error = validate_sms_ir_api_key(cleaned, base_url)
            if error:
                return Response(
        ...
        if profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR:
            SmsConfigService.set_api_key(cleaned, sandbox=True, profile=profile)

    if panel_password is not None and not _is_masked_key(str(panel_password)):
        SmsConfigService.set_panel_password(str(panel_password).strip(), profile=profile)

    profile.refresh_from_db()

    if (
        profile.provider_type == SmsProviderProfile.PROVIDER_IRANPAYAMAK
        and profile.is_active
        and not (profile.line_number or '').strip()
    ):
        return Response(
    ...
    if profile.is_sandbox != was_sandbox and profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR:
        sync_default_template_for_mode(profile.is_sandbox)
...
def _get_profile_or_404(provider_type: str) -> SmsProviderProfile:
...
class AdminSmsProviderProfileListView(APIView):
    permission_classes = [IsStaff]

    def get(self, request):
        SmsConfigService.bootstrap_from_env()
        SmsProviderProfile.ensure_profiles()
        profiles = SmsProviderProfile.objects.all()
        return Response([_profile_response_data(p) for p in profiles])


class AdminSmsProviderProfileDetailView(APIView):
    permission_classes = [IsStaff]

    def get(self, request, provider_type):
        profile = _get_profile_or_404(provider_type)
        return Response(_profile_response_data(profile))

    def patch(self, request, provider_type):
        profile = _get_profile_or_404(provider_type)
        error_response = _apply_profile_settings(request, profile, validate_keys=True)
        if error_response:
...
class AdminSmsProviderProfileActivateView(APIView):
    permission_classes = [IsStaff]

    def post(self, request, provider_type):
        _get_profile_or_404(provider_type)
        profile = SmsConfigService.activate_provider(provider_type)
        if profile.provider_type == SmsProviderProfile.PROVIDER_IRANPAYAMAK:
            sync_iranpayamak_default_template()
        elif profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR:
            sync_default_template_for_mode(profile.is_sandbox)
...
class AdminSmsProviderProfileTestView(APIView):
    permission_classes = [IsStaff]

    def post(self, request, provider_type):
        profile = _get_profile_or_404(provider_type)
        error_response = _apply_profile_settings(request, profile, validate_keys=False)
        if error_response:
            return error_response

        provider = get_sms_provider_for_profile(profile)
        if isinstance(provider, (SmsIrProvider, IranPayamakProvider)):
            result = provider.test_connection()
            profile.last_test_at = timezone.now()
            if result.success:
                profile.last_test_status = SmsProviderProfile.TEST_OK
                credit = result.provider_response.get('credit', 'N/A')
                profile.last_test_message = f'Credit: {credit}'
            else:
                profile.last_test_status = SmsProviderProfile.TEST_FAILED
                profile.last_test_message = result.error or 'Connection failed'
            profile.save(update_fields=['last_test_at', 'last_test_status', 'last_test_message', 'updated_at'])
            return Response({
                'success': result.success,
                'message': profile.last_test_message,
                'response': result.provider_response,
            }, status=status.HTTP_200_OK if result.success else status.HTTP_400_BAD_REQUEST)
        return Response({'success': True, 'message': 'Mock provider — no external connection needed'})


class AdminSmsProviderView(APIView):
    """Backward-compatible shim — returns active profile as legacy provider settings."""
    ...
    def get(self, request):
        ...
        data = _profile_response_data(profile)
        data['provider_mode'] = profile.provider_type
...
class AdminSmsProviderStatusView(APIView):
    ...
    def get(self, request):
        ...
        has_api_key = bool(
            SmsConfigService.resolve_api_key(profile=profile)
            if profile.provider_type == SmsProviderProfile.PROVIDER_SMSIR
            else SmsConfigService.resolve_iranpayamak_api_key(profile=profile)
        )

        return Response({
            'provider_mode': profile.provider_type,
            'is_active': profile.is_active,
            'is_sandbox': profile.is_sandbox,
            'has_api_key': has_api_key,
            'has_production_api_key': bool(SmsConfigService.resolve_api_key(profile=profile, sandbox=False)),
            'has_sandbox_api_key': bool(SmsConfigService.resolve_api_key(profile=profile, sandbox=True)),
            'has_bearer_token': bool(SmsConfigService.resolve_bearer_token(profile=profile)),
            'line_number': profile.line_number,
            'number_format': profile.number_format,
            'panel_username': profile.panel_username,
...
class AdminSmsProviderLoginView(APIView):
    ...
    def post(self, request, provider_type=None):
        profile = (
            _get_profile_or_404(provider_type)
            if provider_type
            else SmsConfigService.get_active_profile()
        )
        if profile.provider_type != SmsProviderProfile.PROVIDER_IRANPAYAMAK:
            return Response({'message': 'ورود پنل فقط برای IranPayamak است'}, status=status.HTTP_400_BAD_REQUEST)

        username = (request.data.get('username') or profile.panel_username or '').strip()
        password = (request.data.get('password') or '').strip()
        if not password:
            password = SmsConfigService.resolve_panel_password(profile=profile)
        if not username or not password:
            return Response({'message': 'نام کاربری و رمز عبور الزامی است'}, status=status.HTTP_400_BAD_REQUEST)

        client, err = _iranpayamak_login_client(profile)
        if err:
            return err
        ...
        if token:
            SmsConfigService.cache_bearer_token(str(token), profile=profile)
            profile.panel_username = username
            if request.data.get('password'):
                SmsConfigService.set_panel_password(password, profile=profile)
            profile.save(update_fields=['panel_username', 'updated_at'])
...
class AdminSmsProviderVerify2faView(APIView):
    ...
    def post(self, request, provider_type=None):
        ...
        SmsConfigService.cache_bearer_token(str(bearer), profile=profile)
        return Response({'success': True, 'message': '2FA تأیید شد — توکن Bearer ذخیره شد'})


def _iranpayamak_login_client(profile: SmsProviderProfile):
    client = get_iranpayamak_client(profile=profile)
    if not client:
