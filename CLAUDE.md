...
**Auth flow:** phone + password. Registration requires `first_name` and `last_name` (the name goes on the parcel) plus a client-side password confirmation; login needs only phone + password. `POST /api/auth/register/` and `/api/auth/login/` both return a JWT pair (access 15 min, refresh 90 days, rotated) — lifetimes live on the `AuthSettings` singleton, editable at `/admin/settings/security`, and every attempt is written to `AuthAuditLog`. There is no OTP and no SMS provider: the only phone numbers the app knows about are the seller's contact handles in `StoreSettings.contact_*`. `ADMIN_PHONES` (comma-separated, default `09916122680,09332279699,09166099383`) lists the phones that become staff/superuser automatically — the entrypoint promotes them on boot and `RegisterSerializer` promotes them at sign-up; the list is mirrored onto `AuthSettings.admin_phones` so it survives without the env var. `ADMIN_BYPASS_PHONE` is narrower: that single phone logs in with any password. `OWNER_PHONE` / `OWNER_NAME` (default `09166099383` / خانم قراچه) name the store owner — the migration seeds them into `StoreSettings.contact_person_name` / `contact_sms_phone` / `contact_whatsapp`, which drive the contact page and the checkout payment buttons.


**Remember this device / auto-login:** both the login and the sign-up form carry a «این دستگاه را به خاطر بسپار» checkbox, pre-ticked according to `AuthSettings.remember_device_default` (read publicly from `GET /api/auth/device/policy/`). Ticking it makes the auth response carry a `device` object — `{id, token, name, expires_at}` — which the client keeps in `localStorage` under `lumia_device`. `TrustedDevice` stores only the SHA-256 of that secret. On boot `stores/auth.ts → hydrateSession()` falls back to `POST /api/auth/device/login/` whenever there is no usable refresh token, so the customer never sees the sign-in form again on that browser; `composables/useApi.ts` does the same on a 401. **The secret is single-use:** every auto-login rotates it and returns the replacement, and `previous_token_hash` is kept for one generation so replaying a spent token is treated as theft — that device is revoked and the user's whole session tree with it (`apps/accounts/services/devices.py`). Customers see and drop their own devices under `/account`; staff see them per user in the dashboard. `apps/accounts/tests_devices.py` locks the rotation, replay and expiry rules.

**Session revocation:** there is no simplejwt blacklist table, so `User.session_epoch` stands in for one. `issue_tokens` stamps it into the token as a `sess` claim (simplejwt copies custom claims across refreshes), `EpochJWTAuthentication` rejects any token whose claim has fallen behind, and `EpochTokenRefreshSerializer` closes the refresh endpoint too. Bumping the counter — `user.revoke_sessions()` — therefore signs a user out of every browser at once. It fires on password change (self or admin), on deactivation, and on a role change (`keep_devices=True` there, since a demoted admin is still a customer).

**User management (`/admin/users`):** staff can create customers, edit any profile field including the phone, toggle `is_active`, reset a password without knowing the old one, sign a user out everywhere, and drop individual remembered devices. Two things are superuser-only and enforced server-side, not just hidden in the UI: granting or revoking `is_staff`/`is_superuser` (`POST /api/admin/users/<id>/roles/` — a plain PATCH cannot touch roles, they are `read_only_fields`) and deleting a user. Staff cannot act on another admin's account at all. The guards live in `apps/admin_api/services/users.py`: nobody may demote, deactivate or delete themselves, the last active superuser is protected, a user with orders is refused deletion (`on_delete=PROTECT`) and pointed at deactivation instead, and demoting a phone that sits on `AuthSettings.admin_phones` is refused with an explanation because the next login would undo it. Editing `admin_phones` / `admin_bypass_phone` is likewise superuser-only — otherwise any staff member could add their own number and be promoted at next login. `apps/admin_api/tests_users.py` locks all of it.
...
| `DRF_THROTTLE_DEVICE_LOGIN` | Rate limit on the passwordless device-login endpoint (default `20/min`) |
