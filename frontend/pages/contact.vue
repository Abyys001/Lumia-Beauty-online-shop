        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-base-content/70 mt-1 dir-ltr text-right">{{ smsPhone }}</p>
            </div>
          </div>
          <div class="flex items-start gap-3">
            <span class="text-xl mt-0.5" aria-hidden="true">⏰</span>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <h3 class="font-bold text-sm text-lumia-dark">ایمیل</h3>
              <a
                href="mailto:support@lumiabeauty.com"
                class="text-xs text-base-content/70 mt-1 font-mono hover:text-lumia-gold transition-colors"
              >
                support@lumiabeauty.com
              </a>
            </div>
          </div>
        </div>
...
const { data: contact } = await usePublicData(
  ...
  {
    default: () => ({ contact_sms_phone: '', contact_telegram: '', contact_whatsapp: '', contact_bale: '' }),
  },
)
...
const smsPhone = computed(() => contact.value?.contact_sms_phone?.trim() || '۰۲۱-۲۲۶۶۰۱۲۰')
...
const channels = computed<SocialChannel[]>(() => {
  ...
  if (contact.value?.contact_bale?.trim()) {
    list.push({
      id: 'bale',
      label: 'بله',
      subtitle: 'ارتباط در پیام‌رسان بله',
      url: `https://bale.me/${handleOf(contact.value.contact_bale, '')}`,
      color: '#00B8A9',
      hoverColor: '#009a8e',
    })
  }
...
useSeoMeta({
  ...
  description: 'ارتباط با لومیا بیوتی از طریق واتس‌اپ، تلگرام، بله و اینستاگرام. مشاوره رایگان انتخاب عطر و روتین پوست.',
})
</script>
